...
int main(int argc, char* argv[]) {
...
#endif
    std::vector<std::vector<omp_lock_t>> locks(gridDim, std::vector<omp_lock_t>(gridDim));
    for (int i = 0; i < gridDim; i++)
        for (int j = 0; j < gridDim; j++) omp_init_lock(&locks[i][j]);
    ...
    std::vector<std::vector<int>> lastSet(gridDim, std::vector<int>(gridDim, -1));
...
#pragma omp parallel for
        for (int i = 0; i < params.param_particles; i++) {
            omp_set_lock(&locks[partCells[i].y][partCells[i].x]);
            if (lastSet[partCells[i].y][partCells[i].x] != step) {
                grid[partCells[i].y][partCells[i].x].clear();
                lastSet[partCells[i].y][partCells[i].x] = step;
            }
            grid[partCells[i].y][partCells[i].x].push_back(i);
            omp_unset_lock(&locks[partCells[i].y][partCells[i].x]);
        }
                ...
                for (int i = 0; i < gridDim; i++)
                    for (int j = 0; j < gridDim; j++) {
                        for (auto& [pai, pbi] : gridColPairs[i][j][0]) {
                            if (pai == -1) {
                                if (is_wall_collision(particles[pbi].loc, particles[pbi].vel, params.square_size,
                                                      params.param_radius)) {
                                    // std::cout << "resolving wall " << pbi << std::endl;
                                    hasCol = true;
                                    resolve_wall_collision(particles[pbi].loc, particles[pbi].vel, params.square_size,
                                                           params.param_radius);
                                }
                            } else if (is_particle_moving_closer(particles[pai].loc, particles[pai].vel,
                                                                 particles[pbi].loc, particles[pbi].vel)) {
                                hasCol = true;
                                resolve_particle_collision(particles[pai].loc, particles[pai].vel, particles[pbi].loc,
                                                           particles[pbi].vel);
                            }
                        }
                    }

// check with right cell, split work by row
#pragma omp parallel for reduction(|| : hasCol)
                for (int i = 0; i < gridDim; i++)
                    for (int j = 0; j < gridDim - 1; j++) hasCol |= colhelper(particles, gridColPairs[i][j][1]);

// check with bottom cell, split work by column
#pragma omp parallel for reduction(|| : hasCol)
                for (int j = 0; j < gridDim; j++)
                    for (int i = 0; i < gridDim - 1; i++) hasCol |= colhelper(particles, gridColPairs[i][j][2]);

// check with br cell, offset 0
#pragma omp parallel for reduction(|| : hasCol)
                for (int i = 0; i < gridDim - 1; i += 2)
                    for (int j = 0; j < gridDim - 1; j++) hasCol |= colhelper(particles, gridColPairs[i][j][3]);

// check with br cell, offset 1
#pragma omp parallel for reduction(|| : hasCol)
                for (int i = 1; i < gridDim - 1; i += 2)
                    for (int j = 0; j < gridDim - 1; j++) hasCol |= colhelper(particles, gridColPairs[i][j][3]);

// check with tr cell, offset 1
#pragma omp parallel for reduction(|| : hasCol)
                for (int i = 1; i < gridDim; i += 2)
                    for (int j = 0; j < gridDim - 1; j++) hasCol |= colhelper(particles, gridColPairs[i][j][4]);

// check with tr cell, offset 2
#pragma omp parallel for reduction(|| : hasCol)
    ...
    for (int i = 0; i < gridDim; i++)
        for (int j = 0; j < gridDim; j++) omp_destroy_lock(&locks[i][j]);
}
